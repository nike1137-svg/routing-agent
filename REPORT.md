...
- **개선 채택 기준**: 한 번에 하나만 바꾸고, 두 지표 중 하나라도 떨어지면 채택하지 않음. 개선 2부터는 같은 구성을 두 번 측정해 **두 회차 모두** 채택 중인 구성보다 떨어지지 않아야 채택
...
"/"는 같은 구성의 1회차 / 2회차입니다. 개선 2부터 실행 간 변동을 확인하려고 두 번씩 측정했습니다. 개선 4~8은 모두 **채택 중인 개선 3**과 비교했습니다.
...
### 수업 내용을 무엇을 반영했고 무엇을 뺐나
처음 제출할 때는 과제 조건과 루브릭에 맞춰 만들었고, 제출 후 수업 예제 코드와 대조해 다시 점검했습니다. 판단 기준은 **이 프로젝트의 실제 문제와 맞는가**였습니다.

| 수업 내용 | 처리 | 이유 |
|---|---|---|
| 분류(모델)와 넘기기 판단(정책)을 다른 노드로 분리 | ✅ 처음부터 반영 | |
| 라우트별로 필요한 장만 골라 근거 조립 | ✅ 처음부터 반영 | 섹션 매핑표 |
| 답변 숫자의 출처를 역추적하는 가드레일 | ✅ 처음부터 반영 | 기계적 검증 |
| 모범 답안으로 채점기 자체를 검증 | ✅ 반영 + 음성 검증 추가 | |
| 평가셋을 프롬프트에 넣지 않기, 한 번에 하나씩, 두 번 재기 | ✅ 반영 | |
| 설정을 `config.py` 한 곳에 | ✅ 제출 후 반영 | 동작 변화 없음 |
| 표현만 바꾼 입력으로 흔들어 보기 | ✅ 제출 후 점검 | 4절. H2-1 취약성 확인 |
| 모델이 도구를 골라 부르는 반복 구조 | 🧪 측정 후 미채택 | 개선 4·5·7·8. 기준 미달이지만 차이는 1~3문항 |
| 분류 원칙 "쓴 단어가 아니라 원하는 결과로 판단" | 🧪 측정 후 미채택 | 개선 6·8. H2-1 분류는 고쳤지만 답변 1문항 하락 |
| 분류 모델과 답변 모델 분리 (luna / terra) | 🧪 측정 후 미채택 | 개선 1·7 |
| 가드레일 위반 시 한 번 더 생성 | ❌ 적용 안 함 | 기존 측정 5회에서 **검증 실패로 넘긴 경우가 0건**이라 달라질 문항이 없음 |
| "애매하면 확신도를 낮춰라" | ❌ 적용 안 함 | 평가셋에 애매해서 넘겨야 하는 문항이 없고, 측정에서 확신도가 임계값 아래로 내려간 경우가 0건 |
| 고정값/조회값 구분, 한 단계 계산값 허용 | ❌ 적용 안 함 | 고객별로 조회할 값과 계산이 필요한 답이 없음 |
| 멀티턴 대화, 되묻기 | ❌ 적용 안 함 | 과제 평가셋이 1턴이라 측정할 수 없음 |
