...
- **필수 사실**: 문항마다 공고문의 **원문 구절과 쪽 번호**를 함께 적었습니다. 구절이 적어 둔 쪽에 실제로 있는지 스크립트로 대조했고, 표에서 나온 사실(추출 텍스트가 깨지는 부분)은 사람이 **PDF 원본과 직접 대조**했습니다.
...
### 작성 과정
1. AI가 공고문 원문을 근거로 30건 초안 작성
2. 사람이 카테고리·난이도 판정을 검토하고, 표에서 나온 사실을 PDF 원본과 대조
3. **예비창업자 말투**의 질문 후보 10개(AI 초안) 중 사람이 5개를 골라, 같은 카테고리·같은 유형 문항 4건과 교체하고 1건을 추가
4. 모범 답안은 확인이 끝난 필수 사실만으로 작성
...
| 대상 | 방법 | 결과 |
|---|---|---|
| **답변 채점기** (LLM, gpt-5.6-luna) | 모범 답안 36건 채점 → 전부 1점이어야 사용 | **36/36** (LLM 채점 24건, 규칙 판정 12건) |
...
| 3 | 답변 규칙: 조건·예외·후속 의무를 빠뜨리지 않음 | 1.000 / 0.968 | 0.968 / 0.968\* | 1.000 / 0.968 | 1.000 / 0.966 | 0.031달러 | ✅ |
...
- **채점기 변동**: 개선 3의 두 회차에서 H1-1 답변 내용이 사실상 같았는데(둘 다 "부도·파산 2년" 없음) **1회차는 실패, 2회차는 통과**로 판정되었습니다. 모범 답안 36/36 검증을 통과해도, 애매한 답변에서는 채점기가 흔들린다는 증거입니다.
...
### 비용
- 전체 개발·측정 누적: **약 0.41달러** (API 호출 488회, 실제 사용 토큰으로 계산)
...
### 왜 이 구조인가
- **순서가 고정된 한 방향 흐름 + 조건부 분기**로 만들었습니다. 과제의 흐름(판정 → 근거 조립 → 답변 → 검증)은 순서가 정해져 있고, 카테고리마다 조회할 근거가 정확히 하나라서 **모델이 도구를 고르게 할 이유가 없었습니다.** 도구를 매핑표대로 코드가 부르므로, 도구 호출 적절성 점수에는 **분류·넘기기 판단의 오류만** 반영됩니다. 모델이 스스로 도구를 고르고 반복하는 구조와의 비교 실험은 하지 않았습니다.
...
### 동작 확인
- 예시 질문 버튼과 채팅 입력창 두 경로 모두 사람이 직접 질문을 넣어 확인했습니다. 브라우저 자동 입력 도구로는 채팅 입력이 서버에 전달되지 않아(API 호출 기록 없음) 자동 확인은 하지 못했습니다.
- 확인한 답변은 공고문과 대조했습니다.
...
- **필수 사실마다 공고문 구절과 쪽을 붙여** 스크립트로 대조하고, 표에서 나온 사실은 사람이 PDF와 대조했습니다.
...
- **데모 자동 테스트**: 지금은 채팅 입력 경로를 사람이 직접 확인했으므로, 자동으로 확인하는 테스트 추가
