...
### 가장 공들인 부분: 수업에서 배운 구조를 내 주제에 옮기기
수업(모두몰 고객응대 에이전트)에서 배운 구조와 측정 방식을 예비창업패키지 공고문에 옮기는 데 가장 공을 들였습니다. 그대로 옮긴 것, 주제에 맞게 바꾼 것, 옮겨서 측정했지만 채택하지 못한 것을 나눠 적습니다.

**그대로 옮긴 것**
- **분류와 넘기기 판단을 다른 노드로 분리**: 수업의 `classify`(모델)·`gate`(정책) 구조를 그대로 가져와, 카테고리 5개와 확신도 임계값으로 적용했습니다.
- **측정 방식**: 평가셋을 프롬프트에 넣지 않기, 모범 답안으로 채점기 자체를 검증하기, 한 번에 하나만 바꾸고 기록하기, 같은 구성을 두 번 재기를 따랐습니다. 다만 두 번 재기는 개선 2의 첫 측정에서 결과가 흔들리는 문항을 보고 나서야 적용했습니다(개선 2 채택은 첫 측정으로 결정).
- **틀린 사례를 직접 읽기**: 수업이 "오분류 목록을 읽는 것이 개선의 출발점"이라고 한 대로, 점수보다 틀린 답변을 먼저 봤습니다. 코딩 에이전트가 틀린 답변과 원인 분류 초안을 뽑고, 저는 실제 답변을 확인해 원인을 확정했습니다. 그 덕에 개선 2의 H2-1 개선과 개선 3의 H1-1 통과가 실행 변동이라는 것을 가려냈습니다.

**주제에 맞게 바꾼 것**
- **라우트별 근거 조립**: 수업은 매뉴얼을 장 단위로 쪼갰지만, 공고문은 한 쪽에 두 카테고리 내용이 섞여 있어 **장·항목 제목 단위 16개 섹션**으로 나눴습니다.
- **가드레일(숫자 출처 역추적)**: 수업은 조회 결과와 대조했지만, 이 주제에는 조회 결과가 없어 **공고문 근거 텍스트**와 대조했습니다. 공고문 날짜 표기와 질문 속 숫자를 인정하도록 고쳐, 맞는 답을 막는 오탐을 기준 측정 전에 없앴습니다.
- **채점기 검증 보강**: 수업의 모범 답안 검증(양성)에 틀린 답을 틀렸다고 하는지 보는 **음성 검증 3종**을 더했고, 근거 문서를 한 버전으로 고정하고 필수 사실마다 공고문 구절과 쪽을 붙여 대조했습니다.

**옮겨서 측정했지만 채택하지 못한 것**
- **모델이 도구를 골라 부르는 구조, 분류 원칙("쓴 단어가 아니라 원하는 결과로 판단"), 분류·답변 모델 분리**를 개선 4~8로 두 번씩 측정했습니다. 분류 오류를 모델이 스스로 바로잡는 장점과 분류 원칙의 효과(H2-1 교정)는 확인했지만, 차이가 1~3문항이라 미리 정한 기준을 넘지 못해 모두 미채택했습니다.
- **입력 흔들어 보기**는 수업이 권한 방법대로 적용해, 평균 점수에서는 보이지 않던 H2-1의 말투 취약성을 찾았습니다.

**가장 아쉬운 점**
전날 수업 실습을 API 키 한도로 끝까지 돌리지 못한 채 과제를 시작했고, 수업 예제와 대조하지 않고 과제 조건과 루브릭에 맞춰 먼저 만들어 제출했습니다. 그래서 수업 구조를 **처음부터 기준선으로 두지 못하고 제출 후 실험으로만** 다뤘습니다. 수업 구조를 기준선으로 시작했다면, 모델이 도구를 고르는 구조가 제 역할을 할 수 있는 자료인지도 주제를 고를 때 함께 봤을 것입니다.
